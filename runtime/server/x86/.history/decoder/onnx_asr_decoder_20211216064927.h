...
class OnnxAsrDecoder {
 ...
 private:
  ...
  Ort::Value InitNullCache(const std::vector<int64_t>& cache_dims, std::vector<float>& cache);
  ...
  Ort::Value subsampling_cache_{nullptr};
  Ort::Value elayers_output_cache_{nullptr};
  Ort::Value conformer_cnn_cache_{nullptr};
