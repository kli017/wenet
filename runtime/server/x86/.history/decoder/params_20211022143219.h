...
// OnnxAsrModel flags
DEFINE_string(encoder_path, "", "onnx encoder model path");
DEFINE_string(ctc_path, "", "onnx ctc model path");
DEFINE_string(decoder_path, "", "onnx decoder model path");
...
std::shared_ptr<OnnxDecodeResource> InitDecodeResourceFromFlags() {
  auto resource = std::make_shared<OnnxDecodeResource>();
...
std::shared_ptr<DecodeResource> InitOnnxDecodeResourceFromFlags() {
  auto resource = std::make_shared<DecodeResource>();

  LOG(INFO) << "Reading torch model " << FLAGS_model_path;
  LOG(INFO) << "Reading onnx encoder model " << FLAGS_encoder_path;
  LOG(INFO) << "Reading onnx ctc model " << FLAGS_ctc_path;
  LOG(INFO) << "Reading onnx decoder model " << FLAGS_decoder_path;
