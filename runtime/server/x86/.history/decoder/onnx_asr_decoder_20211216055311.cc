...
void OnnxAsrDecoder::AttentionRescoring() {
  ...
  std::vector<int64_t> hyps_lengt(num_hyps);
  int max_hyps_len = 0;
  ...
  for (size_t i = 0; i < num_hyps; ++i) {
    const std::vector<int64_t>& hyp = hypotheses[i];
  ...
  std::vecotr<uint8_t> encoder_mask;
  ...
  std::vector<int> encoder_out_onnx_dims{num_hyps, encoder_out.size(1), 256}; // beam, chunk, outsize

  Ort::Value encoder_out_onnx = Ort::Value::CreateTensor<int64_t>(decoder_memory_info,
  ...
  decoder_input.emplace_back(std::move(encoder_out_onnx));
  decoder_input.emplace_back(std::move(encoder_mask_onnx));
  decoder_input.emplace_back(std::move(hyps_tensor_onnx));
  decoder_input.emplace_back(std::move(hyps_lens_onnx);
  ...
  Ort::Env decoder_env_template(ORT_LOGGING_LEVEL_WARNIN, "d"); 
