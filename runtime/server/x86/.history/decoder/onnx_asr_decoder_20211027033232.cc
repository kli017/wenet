...
DecodeState OnnxAsrDecoder::AdvanceDecoding() {
  ...
  if (num_frames >= right_context + 1) {
    ...
    std::string encoder_model_path_template = "/root/wenet-onnx/encoder_chunk_s.onnx";
    ...
    // auto encoder_onnx_outputs1 = session.Run(Ort::RunOptions{nullptr},
    ...
    encoder_outs_.push_back(std::move(chunk_out_tensor));  /// ???
