...
void OnnxAsrDecoder::Reset() {
  ...
  Ort::Value subsampling_cache_init{static_cast<Ort::Value>(nullptr)};
  Ort::Value elayers_output_cache_init{static_cast<Ort::Value>(nullptr)};
  Ort::Value conformer_cnn_cache_init{static_cast<Ort::Value>(nullptr)};
  subsampling_cache_11 = std::move(subsampling_cache_init);
  elayers_output_cache_11 = std::move(elayers_output_cache_init);
  conformer_cnn_cache_11 = std::move(conformer_cnn_cache_init);
  subsampling_cache_ = std::move(InitNullCache(subsampling_cache_dims));
  elayers_output_cache_ = std::move(InitNullCache(elayers_output_cache_dims));
  conformer_cnn_cache_ = std::move(InitNullCache(conformer_cnn_cache_dims));
  encoder_outs_.clear();
...
DecodeState OnnxAsrDecoder::AdvanceDecoding() {
  ...
  if (num_frames >= right_context + 1) {
    ...
    //                               // onnx_model_->encoder_input_node_names().data(),
    //                               encoder_input_names.data(),
    //                               input_onnx.data(),
    //                               input_onnx.size(),
    //                               // onnx_model_->encoder_output_node_names().data(),
    ...
    LOG(INFO) << "encoder out: " << *chunk_out_float << "  " << *(chunk_out_float + 1);
