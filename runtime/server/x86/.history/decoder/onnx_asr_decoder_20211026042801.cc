...
OnnxAsrDecoder::OnnxAsrDecoder(
    ...
    std::shared_ptr<OnnxDecodeResource> resource, const DecodeOptions& opts)
...
Ort::Value OnnxAsrDecoder::InitNullCache(const std::vector<int64_t>& cache_dims) {
  ...
  std::vector<float> init_cache(cache_len, 0.0f);
...
DecodeState OnnxAsrDecoder::AdvanceDecoding() {
  ...
  if (num_frames >= right_context + 1) {
    ...
    size_t num_input_nodes = model_->encoder_module_.GetInputCount();
    size_t num_output_nodes = model_->encoder_module_.GetOutputCount();

    Ort::TypeInfo type_info = model_->encoder_module_.GetInputTypeInfo(i);
    ...
    auto memory_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    Ort::onnx_feats = Ort::Value::CreateTensor<float>( memory_info, 
                                                    feats.data(), 
                                                    feats.size(), 
                                                    input_mask_node_dims.data(),
                                                    input_mask_node_dims.size());

    Ort::onnx_offset = Ort::Value::CreateTensor<int64_t>( offset_, 1, 1, 1);
