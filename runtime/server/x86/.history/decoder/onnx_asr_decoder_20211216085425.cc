...
OnnxAsrDecoder::OnnxAsrDecoder(
  ...
  subsampling_cache_ = std::move(InitNullCache(subsampling_cache_dims, subsampling_cache_vector));
  elayers_output_cache_ = std::move(InitNullCache(elayers_output_cache_dims, elayers_output_cache_vector));
  conformer_cnn_cache_ = std::move(InitNullCache(conformer_cnn_cache_dims, conformer_cnn_cache_vector));
  InitPostProcessing();
}
...
Ort::Value OnnxAsrDecoder::InitNullCache(const std::vector<int64_t>& cache_dims, std::vector<float>& cache) {
  int cache_len = 1;
...
void OnnxAsrDecoder::Reset() {
  ...
  subsampling_cache_ = InitNullCache(subsampling_cache_dims, subsampling_cache_vector);
  elayers_output_cache_ = InitNullCache(elayers_output_cache_dims, elayers_output_cache_vector);
  conformer_cnn_cache_ = InitNullCache(conformer_cnn_cache_dims, conformer_cnn_cache_vector);
  encoder_outs_.clear();
...
DecodeState OnnxAsrDecoder::AdvanceDecoding() {
  ...
  if (num_frames >= right_context + 1) {
    ...
    torch::Tensor ctc_log_probs = torch::from_blob(ctc_probs_float, 
                      {ctc_probs_sizes[1], ctc_probs_sizes[2]}, at::kFloat);
...
void OnnxAsrDecoder::AttentionRescoring() {
  ...
  for (size_t i = 0; i < num_hyps; ++i) {
    ...
    for (size_t j = 0; j < hyp.size(); ++j) {
      hyps_tensor[start_index+j + 1] = hyp[j];
    }
  ...
  {decoder_outputs_first_sizes[0], decoder_outputs_first_sizes[1], decoder_outputs_first_sizes[2]}, at::kFloat);
