...
class OnnxConnectionHandler {
 public:
  OnnxConnectionHandler(tcp::socket&& socket,
                    std::shared_ptr<FeaturePipelineConfig> feature_config,
                    std::shared_ptr<DecodeOptions> decode_config,
                    std::shared_ptr<OnnxDecodeResource> decode_resource_);
  void operator()();
 ...
 private:
  ...
  std::shared_ptr<OnnxAsrDecoder> decoder_ = nullptr;
  std::shared_ptr<std::thread> decode_thread_ = nullptr;
};

class WebSocketServerOnnx {
 public:
  WebSocketServerOnnx(int port,
                  std::shared_ptr<FeaturePipelineConfig> feature_config,
                  std::shared_ptr<DecodeOptions> decode_config,
 ...
 private:
  ...
  std::shared_ptr<DecodeOptions> decode_config_;
  std::shared_ptr<OnnxDecodeResource> decode_resource_;
  WENET_DISALLOW_COPY_AND_ASSIGN(WebSocketServerOnnx);
};
