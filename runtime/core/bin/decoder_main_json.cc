...
int main(int argc, char *argv[]) {
  ...
  for (auto &wav : waves) {
    ...
    std::string file_path = wav.second;
    wenet::SplitStringToVector(file_path, "/", false, &splited_str);
    std::vector<std::string> wavstr;
    wenet::SplitStringToVector(splited_str.back(), ".", false, &wavstr);
    LOG(INFO) << "wavfile: " << wavstr.front();

    CHECK_EQ(wav_reader.sample_rate(), FLAGS_sample_rate);

    std::string json_file = wavstr.front() + ".json";
    ...
    while (true) {
      ...
      if (state == wenet::DecodeState::kEndFeats) {
        ...
        json::object setences;
        auto first_word = decoder.result()[0].word_pieces.front();
        auto last_word = decoder.result()[0].word_pieces.back();
        setences.emplace("setence_id", seg_id);
        setences.emplace("text", decoder.result()[0].sentence);
        setences.emplace("begin_time", first_word.start);
        setences.emplace("end_time", last_word.end);
        segments.emplace_back(setences);
        break;
    ...
    json::value wav_decode_result = { {"file_name", wav.second}, {"result", json::serialize(segments)} };
    LOG(INFO) << "Json: " << json::serialize(wav_decode_result);
    std::string out = json::serialize(wav_decode_result);
    std::ofstream oftream(json_file, std::ios_base::out);
    oftream << out;
    oftream.close();
